...
import { getBlogDetailMetadata } from '@/lib/seo/dynamic';
...
export default async function BlogDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!blogItem || !isContentPublished(blogItem)) {
    notFound();
  }

  const category = blogItem.category || 'Blog';
  const featuredImage = blogItem.featuredImage || blogItem.image || null;
  const dateStr = formatDisplayDate(blogItem.publishDate || blogItem.scheduledDate || blogItem.createdAt, blogItem.date || '');
